...
async function capture(viewport, name) {
  ...
  console.log(`[FRONTLINE_SCREENSHOT_CAPTURE] ${name} ${viewport.width}x${viewport.height} -> ${url}`);
  await page.goto(url, { waitUntil: "networkidle", timeout: 60_000 });
  // Give web fonts a beat to settle even after networkidle.
  await page.waitForTimeout(800);
  const out = join(outDir, `${name}.png`);
  await page.screenshot({ path: out, fullPage: true });
  console.log(`[FRONTLINE_SCREENSHOT_CAPTURE] wrote ${out}`);
  await browser.close();
}
...
console.log("[FRONTLINE_SCREENSHOT_CAPTURE] done.");
