...
> Frontline operates a per-shop voice agent on a real phone number, books appointments into the shop's existing scheduling software, and reports the day's calls back to the owner. This document covers the system topology, data flows, and deploy plan.
...
- **Logs**: `journalctl -u docker.service` on storage-contabo captures container stdout. We grep `[FRONTLINE_NOWPAYMENTS_IPN]` for verified-payment events.
