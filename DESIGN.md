# DESIGN.md — PickupCraft

> Canonical design + style guide for `voice-agents-local` (brand: **PickupCraft**).
...
|-----------|---------------|-------|
| `Logo` | `page.tsx::Logo` | Fraunces 22px "PickupCraft" wordmark next to a 36px ink-rounded-square holding a copper rotary-receiver SVG. The receiver is the brand mark; works at 24px (favicon) and 64px (footer). |
...
**Accessibility for each.** All buttons inherit native focus ring AND have an explicit `:focus-visible { outline: 2px solid var(--copper) }`. The `Logo` carries `aria-label="PickupCraft"`. Industry SVGs are `aria-hidden`; the `Arrow` glyph is `aria-hidden`. Nav anchors are real `<a>` elements via `next/link`. Keyboard tab order: hero CTA → secondary CTA → nav → industry cards → pricing CTAs → footer links.
...
- **Alt text** — `Logo` carries `aria-label="PickupCraft"`; all industry/Arrow/receiver SVGs are `aria-hidden`. Decorative inline glyphs use `aria-hidden`. There are no `<img>` elements; if added, decorative use `alt=""`, content uses descriptive alt.
...
| 2026-05-08 | **Wave 2 rebrand v3 — Rollcall → PickupCraft.** `wave2-name-verdicts.json` (Agent L) flagged Rollcall as FAIL — `rollcall.com` is CQ Roll Call (DC political publication since 1955, very high SEO authority), and SERP for "Rollcall voice AI agent" is dominated by Retell AI + Regal AI direct competitors. Brand was fully eclipsed. Agent Q (Opus 4.7) brainstormed 25 candidates across 5 patterns (compounds, coined words, trade verbs, editorial, cadence) and ran the 4-check methodology (RDAP `.com`, SERP via TinyFish, pronounce/spell, trademark heuristic). Winner: **PickupCraft** (`pickupcraft.com` AVAILABLE per RDAP 404). Verb-of-the-trade ("we pick up") + "craft" = trades audience. SERP for "pickupcraft voice AI" returned zero direct or adjacent competitors; SERP for `"pickupcraft"` exact returned only library "pickup craft" two-word activities (no SaaS conflict). No trademark hits. Rejected candidates: pickupbetty (persona limits gender appeal), tendrline (autocorrect to tender/tendril), warmgreets (warmgreet.com is Indonesian e-commerce on Crunchbase), frontfolk (clean SERP but spelling ambiguity vs frontline/frontfold), openhandphone (long, awkward), oncallcounter (long), warmline (registered), porchpal/neighbordesk/familyline/answerwell (registered .com). Slug + URL `voice-agents-local.prin7r.com` unchanged. Replacement scope: Logo wordmark in `apps/landing/app/page.tsx`, `aria-label`s, `og:title` / `siteName` / metadata, hero deck copy, industries copy, footer attribution, `mailto:` `subject=` strings, `/api/checkout/nowpayments` + `/api/webhooks/nowpayments` log-tag prefixes (`[ROLLCALL_*]` → `[PICKUPCRAFT_*]`), NOWPayments plan `name` + `description` fields, `orderId` prefix (`rollcall_*` → `pickupcraft_*`), `package.json#name` (`rollcall-landing` → `pickupcraft-landing`), all docs, `Dockerfile.landing` header comment, `scripts/capture-landing-screenshots.mjs` log tag. The Soft-Structuralism visual identity (canvas/copper/sage tokens, Fraunces+Cabinet+JetBrains font stack, 6-tile bento, dental transcript, owner's-covenant manifesto) preserved verbatim. Re-deployed to storage-contabo, screenshots re-captured. | Wave 2 rebrand Agent Q |
